...
function ProgressDashboard({ userData }) {
  const [activeTab, setActiveTab] = useState('xp'); // Make sure this is correctly set
  ...
  const streakData = habits.length ? generateStreakData(habits) : [];
  ...
  const longestStreak = activeStreaks.reduce((max, habit) => Math.max(max, habit.streak || 0), 0);
  const streakMultiplier = Math.min(1 + (activeStreaks.length * 0.1), 1.5).toFixed(1);
        ...
        <div className="bg-white p-3 sm:p-4 rounded-xl shadow-md mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center">
            <div className="flex items-center mb-2 sm:mb-0 sm:space-x-4">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold">{username}'s Progress</h2>
                <p className="text-xs sm:text-sm text-gray-600">
                  Level {level} • Total XP: {(level - 1) * 100 + xp} 
                  <span className="hidden xs:inline"> • Active Streaks: {activeStreaks.length || 0}</span>
                </p>
              </div>
            </div>
          </div>
                  ...
                  {activeStreaks.length > 0 ? (
                    <>
                      <li className="flex items-center text-green-600">
                        ...
                        Your most consistent habit is {activeStreaks.sort((a, b) => b.streak - a.streak)[0]?.name || 'none'} with a {longestStreak} day streak!
                      </li>
                      <li className="flex items-center text-orange-500">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M12.395 2.553a1 1 0 00-1.45-.385c-.345.23-.614.558-.822.88-.214.33-.403.713-.57 1.116-.334.804-.614 1.768-.84 2.734a31.365 31.365 0 00-.613 3.58 2.64 2.64 0 01-.945-1.067c-.328-.68-.398-1.534-.398-2.654A1 1 0 005.05 6.05 6.981 6.981 0 003 11a7 7 0 1011.95-4.95c-.592-.591-.98-.985-1.348-1.467-.363-.476-.724-1.063-1.207-2.03zM12.12 15.12A3 3 0 017 13s.879.5 2.5.5c0-1 .5-4 1.25-4.5.5 1 .786 1.293 1.371 1.879A2.99 2.99 0 0113 13a2.99 2.99 0 01-.879 2.121z" clipRule="evenodd" />
                        </svg>
                        You have {activeStreaks.length} active streaks, earning you a {streakMultiplier}x XP multiplier!
                      </li>
                    </>
                  ) : (
